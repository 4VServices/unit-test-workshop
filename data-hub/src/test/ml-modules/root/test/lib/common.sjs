...
function loadDHDatabaseArtifacts(sourceDB, testDB) {
  ...
  const DH_DIRECTORIES = ["/entities/", "/flows/", "/steps/", "/step-definitions/"];
...
function loadDHArtifacts(checkLoadDT = true, seconds = 60) {
  ...
  const lastLoaded = xs.dateTime(xdmp.getServerField(LAST_LOADED_FIELD, now.subtract(expireDuration)));
